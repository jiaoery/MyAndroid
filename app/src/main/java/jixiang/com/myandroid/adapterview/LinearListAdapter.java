...
public class LinearListAdapter extends BaseAdapter {
	...
	public LinearListAdapter(Context context, List<QQFriendBean> list) {
		this.list = list;
	...
	/**
	 * 根据指定的position位置信息，返回当前位置项的行的ID 注意 position 和 ID 是有区别的
	 * (指当给listView设置了header 和footer时)
	 */
	@Override
	...
	/**
	 * 根据指定的position位置信息，从数据集合里面获取到这个位置的数据对象，然后生成一个View（显示项） 并将获取到的数据绑定到这个view上面
	 * position 位置 convertView 缓存的View ViewGroup 父控件
	 */
	@Override
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {// 未使用缓存的View
			//System.out.println("AAAAAAAAAA未使用缓存  position=" + position);
			holder = new Holder();// 初始化一个对象引用保留
			// 新生成一个数据项的View
			convertView = inflater.inflate(R.layout.linearlistview_item, null);
			// 初始化对象保留类里面的属于引用
			...
			//System.out.println("BBBBBBBBBB使用缓存  position=" + position);
	...
	/**
	 * 对每个数据项里面的控件的引用 也是每个数据项里面的控件的保存
